...
/**
 * The result of [AndroidKeyStoreSigner.getAttestation]. A hardware
 * key-attestation certificate chain (`androidKeyChain`) is produced only for a
 * key created with an attestation challenge; otherwise `format` is `none` with
 * an empty chain. The challenge is bound at key generation, never here.
 *
